...
class DifferenceModel(QtCore.QObject):
    ...
    @property
    def width(self):
        return float(self.data.width)

    @property
    def height(self):
        return float(self.data.height)

    @property
    def cx(self):
        return float(self.data.cx)

    @property
    def cy(self):
        return float(self.data.cy)

    @property
    def hint_level(self):
        return int(self.data.hint_level)
    ...
    def click_customized(self):
        return bool(self.data.click_customized)

    @property
    def click_shape(self):
        return self.data.cshape

    @property
    def click_cx(self):
        return float(self.data.ccx)

    @property
    def click_cy(self):
        return float(self.data.ccy)

    @property
    def click_a(self):
        return float(self.data.ca)

    @property
    def click_b(self):
        return float(self.data.cb)
    ...
    def set_click_shape(self, shape: str, *, source=None):
        ...
        if shape == getattr(d, "cshape", "rect"):
            return
        d.cshape = shape
        self.anyChanged.emit(source)
...
class DifferenceItem(QtWidgets.QGraphicsObject):
    ...
    def __init__(
        self, diff: Difference, color: Optional[QtGui.QColor] = None, on_change=None, is_up: bool = True
    ):
        super().__init__()
        ...
        self._cached_rect_size = QtCore.QSizeF(
            max(MIN_RECT_SIZE, float(self.model.width)), max(MIN_RECT_SIZE, float(self.model.height))
        )
    ...
    def _click_handles(self, c: QtCore.QPointF, a: float, b: float, shape: str):
        ...
        return {"TL": TL, "TR": TR, "BR": BR, "BL": BL, "L": L, "R": R, "T": T, "B": B}
    ...
    def _draw_badge(
        ...
        grad = QtGui.QLinearGradient(
            badge.center().x() - w / 2, badge.center().y(), badge.center().x() + w / 2, badge.center().y()
        )
    ...
    def paint(self, p: QtGui.QPainter, option, widget=None):
        ...
        if self._show_circle:
            ...
            if self._hl_circle:
                ...
                path_outer.addEllipse(outer)
                path_inner = QtGui.QPainterPath()
                path_inner.addEllipse(bbox)
        ...
        if self.model.click_customized and self._show_click:
            ...
            hs = self.HANDLE_SIZE
            p.setPen(self.HANDLE_PEN)
            p.setBrush(self.HANDLE_BR_CLICK)
    ...
    def _refresh_bounds_if_needed(self):
        ...
        if (
            abs(new.x() - old.x()) > 1e-6
            or abs(new.y() - old.y()) > 1e-6
            or abs(new.width() - old.width()) > 1e-6
            or abs(new.height() - old.height()) > 1e-6
        ):
    ...
    def hoverMoveEvent(self, e: QtWidgets.QGraphicsSceneHoverEvent) -> None:
        ...
        if self._can_hit_click():
            ...
            if hcode:
                ...
                self._set_hover_state(rect_hl=False, circ_hl=False, click_hl=True)
                return
    ...
    def _set_hover_state(self, rect_hl: bool, circ_hl: bool, click_hl: bool = False):
        ...
        if self._hl_rect != rect_hl:
            self._hl_rect = rect_hl
            changed = True
        if self._hl_circle != circ_hl:
            self._hl_circle = circ_hl
            changed = True
        if self._hl_click != click_hl:
            self._hl_click = click_hl
            changed = True
        if changed:
            self.update()
    ...
    def mousePressEvent(self, e: QtWidgets.QGraphicsSceneMouseEvent):
        ...
        if self._can_hit_click():
            ...
            if hcode:
                self._mode = (
                    self.Mode.CLICK_EDGE if hcode in ("L", "R", "T", "B") else self.Mode.CLICK_CORNER
                )
                self._click_hcode = hcode
                self._click_press_local = e.pos()
                (
                    self._click_press_center,
                    self._click_press_a,
                    self._click_press_b,
                    self._click_press_shape,
                ) = self._current_click_local()
        ...
        if self._can_hit_rect():
            ...
            if corner >= 0:
                ...
                # 对角锚点
                opp = [
                    QtCore.QPointF(br_scene.x(), br_scene.y()),
    ...
    def mouseMoveEvent(self, e: QtWidgets.QGraphicsSceneMouseEvent):
        ...
        if self._mode == self.Mode.RESIZE_CORNER:
            cur = e.scenePos()
            tl_scene = QtCore.QPointF(
                min(cur.x(), self._anchor_scene.x()), min(cur.y(), self._anchor_scene.y())
            )
            br_scene = QtCore.QPointF(
                max(cur.x(), self._anchor_scene.x()), max(cur.y(), self._anchor_scene.y())
            )
            ...
            x, y, w, h = (
                tl_scene.x(),
                tl_scene.y(),
                br_scene.x() - tl_scene.x(),
                br_scene.y() - tl_scene.y(),
            )
    ...
    def itemChange(self, change, value):
        ...
        if change == QtWidgets.QGraphicsItem.ItemPositionHasChanged:
            ...
            if self._rect_interactions_allowed():
                rect = self._current_rect_local()
                self.model.set_rect(
                    self.pos().x(), self.pos().y(), rect.width(), rect.height(), source=self
                )
    ...
    def _hit_edge(self, rect: QtCore.QRectF, pos: QtCore.QPointF) -> str:
        ...
        if 0 <= pos.y() <= rect.height():
            if abs(pos.x() - 0.0) <= et:
                return "L"
            if abs(pos.x() - rect.width()) <= et:
                return "R"
        if 0 <= pos.x() <= rect.width():
            if abs(pos.y() - 0.0) <= et:
                return "T"
            if abs(pos.y() - rect.height()) <= et:
                return "B"
        return ""
    ...
    def _clamp_scene_rect(
        self, tl: QtCore.QPointF, br: QtCore.QPointF, scene_rect: QtCore.QRectF
        ...
        br = QtCore.QPointF(min(scene_rect.right(), tl.x() + w), min(scene_rect.bottom(), tl.y() + h))
        return tl, br
    ...
    def setVis(self, show_click: bool, show_rect: bool, show_circle: bool, show_label: bool):
        ...
        changed = False
        if self._show_click != bool(show_click):
            self._show_click = bool(show_click)
            changed = True
        if self._show_rect != bool(show_rect):
            self._show_rect = bool(show_rect)
            changed = True
        if self._show_circle != bool(show_circle):
            self._show_circle = bool(show_circle)
            changed = True
        if self._show_label != bool(show_label):
            self._show_label = bool(show_label)
            changed = True
        if changed:
            # 关闭矩形时去掉矩形高亮；关闭圆时去掉圆高亮
            if not self._show_rect:
                self._hl_rect = False
            if not self._show_circle:
                self._hl_circle = False
            if not self._show_click:
